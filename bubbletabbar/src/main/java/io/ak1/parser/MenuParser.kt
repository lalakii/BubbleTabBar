...
internal class MenuParser(private val context: Context) {
    ...
    private fun parseMenu(
        ...
        while (!isEndOfMenu) {
            val name = parser.name
            when {
                eventType == XmlPullParser.START_TAG && name == XML_MENU_ITEM_TAG -> {
                    val item = parseMenuItem(attrs)
                    items.add(
                        item,
                    )
                }
                eventType == XmlPullParser.END_TAG && name == XML_MENU_TAG -> isEndOfMenu = true
                eventType == XmlPullParser.END_DOCUMENT -> throw NullPointerException("Unexpected end of document")
            }
